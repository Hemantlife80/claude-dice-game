...
const Banner728x90Ad: React.FC<{ adKey: string }> = React.memo(({ adKey }) => {
    ...
    try {
      ...
      script.innerHTML = `
        (function() {
          ...
          } catch(e) { console.log('Ad failed to load', e); }
        })();
      `;
  ...
  return <div id={containerId} className="flex justify-center items-center w-full h-[90px] bg-gray-100 border-b border-gray-300"></div>;
});
...
const NativeBannerAd: React.FC<{ adKey: string }> = React.memo(({ adKey }) => {
  ...
  useEffect(() => {
    try {
      ...
      script.onerror = () => console.log('Native ad failed to load');
      adContainer.appendChild(script);
    } catch (e) {
      console.log('Native ad error:', e);
    }
  }, [nativeContainerId]);
  
  return <div id={nativeContainerId} className="flex justify-center items-center w-full min-h-[100px] bg-gray-100 border-t border-gray-300"></div>;
});
...
const DiceGame: React.FC = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      diceRef.current.castShadow = true;
      diceRef.current.receiveShadow = true;
            ...
            <div className="bg-white bg-opacity-95 rounded-xl shadow-lg p-4">
              ...
              <div className="mt-4 pt-4 border-t border-gray-300">
                <p className="text-sm text-gray-700">
                  Target: <span className="font-bold">{targetScore}</span>
                </p>
              </div>
            </div>
            <div className="bg-white bg-opacity-95 rounded-xl shadow-lg p-4">
                ...
                {rollHistory.length === 0 ? (
                  ...
                  rollHistory.map((roll, i) => (
                    <div key={i} className="flex justify-between items-center bg-gray-100 p-2 rounded-lg">
                      ...
                      <span className="bg-amber-600 text-white px-3 py-1 rounded-full font-bold text-sm">
                        {roll[1]}
                      </span>
                    </div>
                  ))
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-4">
            ...
            <div className="text-center">
              <p className="text-white text-lg mb-2">
                Current Player: <span className="font-bold text-amber-300">{playerNames[currentPlayer]}</span>
              </p>
              {voiceMessage && (
                <p className="text-yellow-200 text-sm mb-2 bg-black bg-opacity-50 p-2 rounded-lg">
                  {voiceMessage}
                </p>
              )}
  ...
  return (
    <div
      ...
      style={{
        backgroundImage:
          'url("data:image/svg+xml,%3Csvg width=\'100\' height=\'100\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Crect fill=\'%23744210\' width=\'100\' height=\'100\'/%3E%3Cpath fill=\'%23654321\' d=\'M0 0h100v50H0z\'/%3E%3Cpath fill=\'%23845432\' d=\'M20 20h60v60H20z\'/%3E%3C/svg%3E")'
      }}
    >
